...
public class CuWateringcanItemBuilder extends HandheldItemBuilder {
    private int chance = 100;
    private int range = 1;

    public CuWateringcanItemBuilder(ResourceLocation i) {
        super(i, 0.5f, -2f);
    }

    @Info(value = "The range of farmland the watering can can water around the center")
    public CuWateringcanItemBuilder range(int range) {
        this.range = range;
        return this;
    }
    @Info(value = "The chance that the watering can bonemeals a crop per each use")
    public CuWateringcanItemBuilder chance(int chance) {
        this.chance = chance;
        return this;
    }
    @Override
